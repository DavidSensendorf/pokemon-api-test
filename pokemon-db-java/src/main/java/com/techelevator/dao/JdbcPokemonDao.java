...
public class JdbcPokemonDao implements PokemonDao{
    ...
    public PokemonDetail saveFavorites(PokemonDetail detail, int userId) {
        String sql = "INSERT INTO pokemon (api_id, name, base_experience, height, weight, back_url, front_url)" +
                "VALUES(?, ?, ?, ?, ?, ?, ?) RETURNING id";
        int id = template.queryForObject(sql, int.class, detail.getApiId(), detail.getName(), detail.getBaseExperience(),
                detail.getHeight(), detail.getWeight(), detail.getSprite().getBackDefault(), detail.getSprite().getFrontDefault());
